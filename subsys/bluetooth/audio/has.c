...
static void notify_work_reschedule(struct has_client *client, enum has_notify notify,
				   k_timeout_t delay)
{
	int err;

	atomic_set_bit(client->notify, notify);
...
static void notify_work_handler(struct k_work *work)
{
	struct has_client *client = CONTAINER_OF(work, struct has_client, notify_work);
	int err;

#if defined(CONFIG_BT_HAS_FEATURES_NOTIFIABLE)
	if (atomic_test_and_clear_bit(client->notify, NOTIFY_FEATURES) &&
	    bt_gatt_is_subscribed(client->conn, FEATURES_ATTR, BT_GATT_CCC_NOTIFY)) {
		err = bt_gatt_notify(client->conn, FEATURES_ATTR, &has.features,
				     sizeof(has.features));
		if (err == -ENOMEM) {
			notify_work_reschedule(client, NOTIFY_FEATURES,
...
#if defined(CONFIG_BT_HAS_PRESET_SUPPORT)
	if (atomic_test_and_clear_bit(client->notify, NOTIFY_READ_PRESET_RESPONSE)) {
		err = read_preset_response(client);
		if (err == -ENOMEM) {
			notify_work_reschedule(client, NOTIFY_READ_PRESET_RESPONSE,
					       K_USEC(BT_AUDIO_NOTIFY_RETRY_DELAY_US));
		} else if (err < 0) {
			LOG_ERR("Notify read preset response err %d", err);
		}
	} else if (atomic_test_and_clear_bit(client->notify, NOTIFY_CURRENT_PRESET_LIST)) {
		err = list_current_presets(client);
		if (err == -ENOMEM) {
			notify_work_reschedule(client, NOTIFY_CURRENT_PRESET_LIST,
					       K_USEC(BT_AUDIO_NOTIFY_RETRY_DELAY_US));
		} else if (err < 0) {
...
static void notify(struct has_client *client, enum has_notify notify)
{
	if (client != NULL) {
		notify_work_reschedule(client, notify, K_NO_WAIT);
		return;
	}
...
static struct has_preset *preset_get_prev(const struct has_preset *preset, bool wrap)
{
	struct has_preset *tmp, *prev = NULL;
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&preset_list, tmp, node) {
		if (tmp->index == preset->index) {
			if (prev == NULL && wrap) {
				sys_snode_t *node = sys_slist_peek_tail(&preset_list);

				prev = CONTAINER_OF(node, struct has_preset, node);
				if (prev == preset) {
					return NULL;
				}
			}
...
static struct has_preset *preset_get_next(struct has_preset *preset, bool wrap)
{
	struct has_preset *next;

	next = SYS_SLIST_PEEK_NEXT_CONTAINER(preset, node);
	if (next == NULL && wrap) {
		next = SYS_SLIST_PEEK_HEAD_CONTAINER(&preset_list, next, node);
		if (next == preset) {
			return NULL;
		}
	}
...
static uint8_t preset_get_prev_index(const struct has_preset *preset)
{
	...
	prev = preset_get_prev(preset, false);
	if (prev == NULL) {
...
static void control_point_ntf_complete(struct bt_conn *conn, void *user_data)
{
	...
	/* Resubmit if needed */
	if (client != NULL && atomic_get(client->notify) != 0) {
		k_work_reschedule(&client->notify_work, K_NO_WAIT);
	}
}
...
static int control_point_send_all(struct net_buf_simple *buf)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(has_client_list); i++) {
		...
		if (!client->conn) {
			...
			atomic_set_bit(client->notify, NOTIFY_CURRENT_PRESET_LIST);
...
static uint8_t handle_read_preset_req(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	if (atomic_test_bit(client->notify, NOTIFY_READ_PRESET_RESPONSE)) {
	...
	notify(client, NOTIFY_READ_PRESET_RESPONSE);
...
static void preset_set_active(struct has_preset *preset)
{
	if (active_preset != preset) {
		...
		notify(NULL, NOTIFY_ACTIVE_INDEX);
	}
}
...
{
	return preset->properties & BT_HAS_PROP_AVAILABLE;
}
...
static uint8_t handle_set_next_preset(bool sync)
{
	...
	if (active_preset == NULL) {
		...
		next = preset_get_next(active_preset, true);
	}
	...
	do {
		...
		next = preset_get_next(next, true);
	} while (tmp != next);
...
static uint8_t handle_set_prev_preset(bool sync)
{
	...
	if (active_preset == NULL) {
		...
		prev = preset_get_prev(active_preset, true);
	}
	...
	do {
		...
		prev = preset_get_prev(prev, true);
	} while (tmp != prev);
